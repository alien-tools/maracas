...
public class ClassLessAccessibleVisitor extends BreakingChangeVisitor {
    ...
    public <T> void visitCtTypeReference(CtTypeReference<T> reference) {
        if (clsRef.equals(reference)) {
            ...
            case PROTECTED:
                if (// Is not subtype of clsRef?
                    (!reference.equals(clsRef) && !reference.isSubtypeOf(clsRef) &&
                    !enclosingPkg.equals(expectedPkg)) ||
                    // Is it explicitly calling the constructor of the inner class?
                    (reference.equals(clsRef) &&
                    reference.getParent() instanceof CtExecutableReference<?> execRef
                    && execRef.isConstructor() && execRef.getParent() != null &&
                    !execRef.getParent().isImplicit()))
                    brokenUse(reference.getParent(), reference, clsRef, use);
                break;
            default:
                // Can't happen
